"""Generates Course_Plan_Python_Lab.docx from the same data as the PDF."""
...
UNITS, COURSE, FACULTY = ns["UNITS"], ns["COURSE"].replace("&amp;", "&"), ns["FACULTY"]
exec("modes = [" + src.split("modes = [")[1].split("]\n")[0] + "]", ns)
MODES = ns["modes"]
OUT = __file__.replace("build_course_plan_docx.py", "Course_Plan_Python_Lab.docx")
...
# ---- cover page
para(before=200)
for text, size, bold, gap in [("COURSE PLAN", 18, True, 8), (COURSE, 22, True, 60),
...
p = para(after=2)
for txt, b in [("Total Hours: ", True), ("45 (15 sessions × 3 hours)", False)]:
    r = p.add_run(txt); r.bold = b; r.font.size = Pt(9.5)
p = para(after=2)
for txt, b, it in [("Software / Tools: ", True, False),
                   ("Python 3, Jupyter Notebook / IDLE, NumPy, Pandas, SciPy, Matplotlib, scikit-learn "
                    "(Anaconda distribution); Linux or Windows", False, False)]:
    r = p.add_run(txt); r.bold, r.italic = b, it; r.font.size = Pt(9.5)
p = para(after=2)
for txt, b, it in [("Reference Books: ", True, False),
                   ("(1) Wesley J. Chun, ", False, False), ("Core Python Programming", False, True), (", 2/e, Prentice Hall; ", False, False),
                   ("(2) Chris Albon, ", False, False), ("Machine Learning with Python Cookbook", False, True), (", O'Reilly, 2018; ", False, False),
                   ("(3) Mark Summerfield, ", False, False), ("Programming in Python 3", False, True), (", 2/e, Addison-Wesley; ", False, False),
                   ("(4) Phuong Vo.T.H, Martin Czygan, ", False, False), ("Getting Started with Python Data Analysis", False, True), (", Packt; ", False, False),
                   ("(5) Armando Fandango, ", False, False), ("Python Data Analysis", False, True), (", Packt; ", False, False),
                   ("(6) Magnus Vilhelm Persson, Luiz Felipe Martins, ", False, False), ("Mastering Python Data Analysis", False, True), (", Packt; ", False, False),
                   ("(7) Sebastian Raschka, Vahid Mirjalili, ", False, False), ("Python Machine Learning", False, True), (", Packt", False, False)]:
    r = p.add_run(txt); r.bold, r.italic = b, it; r.font.size = Pt(9.5)
...
for num, name, topics in UNITS:
    ...
    table(["S.No.", "Experiment / Topic", "Hrs", "Mode of Teaching", "Learning Activity"],
...
doc.core_properties.title = "Python Programming Lab – Course Plan"
