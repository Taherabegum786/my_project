"""Generates Course_Plan_Python_Programming.docx from the same data as the PDF."""
...
MODES = ns["modes"]
OUT = __file__.replace("build_course_plan_docx.py", "Course_Plan_Python_Programming.docx")
...
for txt, b, it in [("Software / Tools: ", True, False),
                   ("Python 3, Jupyter Notebook / IDLE, pip, NumPy, Pandas, Matplotlib (Anaconda distribution)", False, False)]:
    r = p.add_run(txt); r.bold, r.italic = b, it; r.font.size = Pt(9.5)
p = para(after=2)
for txt, b, it in [("Text Books: ", True, False), ("(1) Mark Summerfield, ", False, False),
                   ("Programming in Python 3: A Complete Introduction to the Python Language", False, True),
                   (", 2/e, Addison-Wesley; (2) Phuong Vo.T.H, Martin Czygan, ", False, False),
                   ("Python: End-to-End Data Analysis Learning Path, Module 1: Getting Started with Python Data Analysis", False, True),
                   (", Packt", False, False)]:
    r = p.add_run(txt); r.bold, r.italic = b, it; r.font.size = Pt(9.5)
...
doc.core_properties.title = "Python Programming – Course Plan"
