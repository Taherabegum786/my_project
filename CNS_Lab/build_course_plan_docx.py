"""Generates Course_Plan_CNS_Lab.docx from the same data as the PDF."""
...
MODES = ns["modes"]
OUT = __file__.replace("build_course_plan_docx.py", "Course_Plan_CNS_Lab.docx")
...
for txt, b, it in [("Software / Tools: ", True, False),
                   ("GCC, JDK (javax.crypto, keytool), web browser, Wireshark, tcpdump / WinDump, Nmap, Snort, "
                    "arpspoof / Ettercap; Linux or Windows", False, False)]:
    r = p.add_run(txt); r.bold, r.italic = b, it; r.font.size = Pt(9.5)
p = para(after=2)
for txt, b, it in [("Reference Books: ", True, False), ("(1) William Stallings, ", False, False),
                   ("Computer Security – Principles and Practices", False, True),
                   (", 2/e, Pearson Education; (2) William Stallings, ", False, False),
                   ("Cryptography and Network Security", False, True), (", Pearson Education Asia, New Delhi", False, False)]:
    r = p.add_run(txt); r.bold, r.italic = b, it; r.font.size = Pt(9.5)
...
doc.core_properties.title = "Cryptography & Network Security Lab – Course Plan"
